...
class CNNEncoder(nn.Module):
      def __init__(self, in_channels, out_channels, max_len = 100):
        ...
        self.GRU = nn.GRU(input_size=in_channels,hidden_size=out_channels,num_layers=1, bidirectional=True, batch_first=True)
...
class CNNEncoder_ori(nn.Module):
    def __init__(self, in_channels, out_channels, max_len = 100):
        super(CNNEncoder, self).__init__()
        self.pos_embedding = nn.Parameter(torch.randn(1, max_len + 1, in_channels))
        self.cnn = nn.Conv1d(in_channels, out_channels, 3, 1, padding=1)
...
class MetricSum(nn.Module):
    ...
    def __init__(self, opts, word2vec=None):
        ...
        self.document_encoder = DocumentEncoder(150, 300*2, 3, 6, 300*4, dropout=0.5)
        # dim = 300
        # mlp_dim = 2*dim
        self.classifier = nn.Linear(300*2, 1)
